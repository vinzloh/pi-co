...
const DEFAULT_CONFIG: CloakConfig = {
  ...
  patterns: [
    {
      "filePattern": [".env", "**/.env.*", "config/*.json", "*.secret.*"],
      "cloakPattern": [
        // Match KEY=value pairs, keep key visible, mask the value
        // $1 = key=, $2 = value
        {
          "pattern": "((?:API|SECRET|PRIVATE|ACCESS|REFRESH|AUTH|SESSION)_[A-Z_]*=)(\\S+)",
          "replace": "$1",
        },
        {
          "pattern": "([A-Z_]*(?:SIGN|ID|PASSWORD|TOKEN|KEY|SECRET|CREDENTIALS)=)(\\S+)",
          "replace": "$1",
        },
        // URLs with credentials: keep protocol://user: visible, mask password
        {
          "pattern": "((?:postgres|mysql|mongodb|redis|https?|ftp)://[^\\s\"'/]*:)([^\\s\"'/@]*)(@[^\\s\"']+)",
          "replace": "$1",
        },
      ]
    },
